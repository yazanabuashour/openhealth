...
This project is pre-`1.0` and ships a production `openhealth` runner, a
single-file OpenHealth skill, and a direct-local Go package. The supported
code lines are the current default branch and the most recent `0.y.z` tag, if
one exists.
...
checksums, SBOMs, and GitHub attestations. OpenHealth does not publish a hosted
