...
func validateReleaseDocs(root string, tag string) error {
	...
	releaseURL := "https://github.com/yazanabuashour/openhealth/releases/tag/" + tag
...
func validateReleaseNotes(notesPath string, content string, tag string) error {
	...
	if len(lines) == 0 || strings.TrimSpace(lines[0]) != "# OpenHealth "+tag {
		return fmt.Errorf("%s must start with %q", notesPath, "# OpenHealth "+tag)
	}
