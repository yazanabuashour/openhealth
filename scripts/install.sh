...
repo="yazanabuashour/openhealth"
default_version="__OPENHEALTH_VERSION__"
...
select_version() {
  requested="${OPENHEALTH_VERSION:-$default_version}"
  case "$requested" in
    "" | "__OPENHEALTH_VERSION__" | latest)
      resolve_latest_version
      ;;
    v*)
      printf '%s' "$requested"
...
select_install_dir() {
  if [ -n "${OPENHEALTH_INSTALL_DIR:-}" ]; then
    printf '%s' "$OPENHEALTH_INSTALL_DIR"
    return
  fi
...
tag="$(select_version)"
asset_version="${tag#v}"
archive="openhealth_${asset_version}_${os}_${arch}.tar.gz"
checksum="openhealth_${asset_version}_checksums.txt"
release_url="https://github.com/${repo}/releases/download/${tag}"
tmp_dir="$(mktemp -d "${TMPDIR:-/tmp}/openhealth-install.XXXXXX")"
...
log "Installing OpenHealth ${tag} for ${os}/${arch}"
